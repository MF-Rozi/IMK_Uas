...
while ($row = mysqli_fetch_array($run_query)) {
	...
	echo "<td><a href='?change_to_author={$user_id}'><span class='glyphicon glyphicon-off' style='color: #265a88;'></span></a></td>";
...
if (isset($_GET['change_to_admin'])) {
	...
	if ($_SESSION['role'] != 3) {
		...
		$publish_query = "UPDATE users SET user_role= 3 WHERE user_id={$user_admin}";
...
if (isset($_GET['change_to_sub'])) {
	...
	if ($_SESSION['role'] != 3) {
		...
		$draft_query = "UPDATE users SET user_role= 1 WHERE user_id={$user_sub}";
...
?>
<?php
if (isset($_GET['change_to_author'])) {
	$user_sub = $_GET['change_to_author'];
	if ($_SESSION['role'] != 3) {
		header('Location: ./user_error.php');
	} else {
		$draft_query = "UPDATE users SET user_role=2 WHERE user_id={$user_sub}";
		$run_draft_query = mysqli_query($con, $draft_query);
		header('Location:users.php');
	}
